  ...
  <el-card>
    ...
    <el-row>
      ...
      <el-col :offset="2" :span="4">
        <!--
          el-upload上传组件，它会自动将用户选择的图片区请求上传，我们要做的
          就是配置一下action请求地址
          由于它用的自己内部的请求，不是使用的axios去发请求
          完整路径
          它的请求也不会经过axios拦截器，所以需要手动配置token
...
export default {
  ...
  methods: {
    ...
    handleUpload(uploadConfig) {
      // axios上传文件
      //  1 构建一个表单对象   将文件对象添加到formData中
      //  2 将formData 配置到请求体data选项中
